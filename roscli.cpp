...
bool check_node(char* nodename) {
    char buffer[1000];
    snprintf(buffer, 1000, "/opt/ros/noetic/bin/./rosnode ping -c 4 %s 2>&1 | tee stat.op", nodename);
    system(buffer);

    std::ifstream stat_str("stat.op");
    std::string stat_buf((std::istreambuf_iterator<char>(stat_str)),
                     std::istreambuf_iterator<char>());
    std::string output = stat_buf.c_str();
    qDebug() << "BHAIJAAN" << output.c_str();
    if ((output.find("No")) || (output.find("ERROR"))  != std::string::npos) {
        return false;
    }
    return true;
}
...
void rosCLI::init() {
    ...
    // #FIXME DO ROS CHECK
    bool stat_cam = check_node("camera");
